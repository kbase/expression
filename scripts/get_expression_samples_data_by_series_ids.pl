...
=head1 get_expression_samples_data_by_series_ids

Example:

    get_expression_samples_data_by_series_ids [arguments] < input > output
...
This script is a wrapper for the CDMI-API call get_expression_samples_data_by_series_ids. It is documented as follows:
...
my $usage = "usage: get_expression_samples_data_by_series_ids [-c column] < input > output";
...
while (my @tuples = Bio::KBase::Utilities::ScriptThing::GetBatch($ih, undef, $column)) {
    my @h = map { $_->[0] } @tuples;
    my $h = $kbO->get_expression_samples_data_by_series_ids(\@h);
    for my $tuple (@tuples) {
        #
